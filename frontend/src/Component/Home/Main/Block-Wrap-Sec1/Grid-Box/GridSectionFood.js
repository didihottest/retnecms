...
// component
import { sentChildFoodFilter } from './redux/actions';

const GridSectionFood = (props)=>{
const {category, imgHm,
        title, date, 
    user,comment,
    view,imgSt,articleId} = props

	const dispatch = useDispatch();

	 // DISPATCH API TO ACTION
	useEffect(()=>{
		dispatch(sentChildFoodFilter())
	},[dispatch])

	 // SELECTOR 
	const getChildFashion = useSelector(state=> state.articlesFilter)
	const {childFoodArticle} = getChildFashion
    ...
    return  <div className="col-md-6">
					...
					<div className="row">
					{childFoodArticle && childFoodArticle.map((article)=>{
...
export default GridSectionFood;